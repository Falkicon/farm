  ...
  (function(module, exports) {
    (function(global, factory) {
      ...
      function getNumber(text) {
        var _a, _b;
        return (_b = (_a = numberRegex.exec(text)) === null || _a === void 0 ? void 0 :
        _a[0]) !== null && _b !== void 0 ? _b : null;
      }
      ...
      function assertRootResult(result) {
        ...
        if (result.type === "JsdocTypeKeyValue" || result.type === "JsdocTypePar\
ameterList" || result.type === "JsdocTypeProperty" || result.type === "JsdocType\
ReadonlyProperty" || result.type === "JsdocTypeObjectField" || result.type === "\
JsdocTypeJsdocObjectField" || result.type === "JsdocTypeIndexSignature" || result.
      ...
      function assertNumberOrVariadicNameResult(result) {
        ...
        if (result.type === "JsdocTypeVariadic") {
          if (((_a = result.element) === null || _a === void 0 ? void 0 : _a.type) ===
          "JsdocTypeName") {
            return result;
          }
          throw new UnexpectedTypeError(result);
      ...
      function isSquaredProperty(result) {
        return result.type === "JsdocTypeIndexSignature" || result.type === "Jsd\
ocTypeMappedType";
      }
      var Precedence;
      ...
      function isQuestionMarkUnknownType(next) {
        return next === "EOF" || next === "|" || next === "," || next === ")" ||
        next === ">";
      }
      const nullableParslet = (parser, precedence, left) => {
        const type = parser.lexer.current.type;
        const next = parser.lexer.next.type;
        const accept = left == null && type === "?" && !isQuestionMarkUnknownType(
        next) || left != null && type === "?";
        if (!accept) {
          return null;
        }
      ...
      function createParameterListParslet({ allowTrailingComma }) {
        return composeParslet({
          ...
          parseInfix: (parser, left) => {
            ...
            if (elements.length > 0 && elements.slice(0, -1).some((e) => e.type ===
            "JsdocTypeVariadic")) {
      ...
      function createNamePathParslet({ allowSquareBracketsOnAnyType, allowJsdocNamePaths,
      pathGrammar: pathGrammar2 }) {
        return function namePathParslet(parser, precedence, left) {
          ...
          const type = parser.lexer.current.type;
          const next = parser.lexer.next.type;
          const accept = type === "." && next !== "<" || type === "[" && (allowSquareBracketsOnAnyType ||
          left.type === "JsdocTypeName") || allowJsdocNamePaths && (type === "~" ||
          type === "#");
          if (!accept) {
            return null;
          }
          ...
          const pathParser = pathGrammar2 !== null ? new Parser(pathGrammar2, parser.
          lexer, parser) : parser;
          ...
          switch (parsed.type) {
            ...
            case "JsdocTypeSpecialNamePath":
              if (parsed.specialType === "event") {
                ...
                throw new UnexpectedTypeError(parsed, "Type 'JsdocTypeSpecialNam\
ePath' is only allowed with specialType 'event'");
              }
              break;
            default:
              throw new UnexpectedTypeError(parsed, "Expecting 'JsdocTypeName', \
'JsdocTypeNumber', 'JsdocStringValue' or 'JsdocTypeSpecialNamePath'");
          }
          if (brackets && !parser.consume("]")) {
            const token = parser.lexer.current;
            throw new Error(`Unterminated square brackets. Next token is '${token.
            type}' with text '${token.text}'`);
          }
          return {
      ...
      function createNameParslet({ allowedAdditionalTokens }) {
        return composeParslet({
          name: "nameParslet",
          accept: (type) => type === "Identifier" || type === "this" || type ===
          "new" || allowedAdditionalTokens.includes(type),
          parsePrefix: (parser) => {
      ...
      function createSpecialNamePathParslet({ pathGrammar: pathGrammar2, allowedTypes }) {
        return composeParslet({
          ...
          parsePrefix: (parser) => {
            ...
            const moduleResult = moduleParser.parseInfixIntermediateType(result,
            Precedence.ALL);
      ...
      allowNewAsFunctionKeyword }) {
        return composeParslet({
          ...
          accept: (type, next) => type === "function" || allowNewAsFunctionKeyword &&
          type === "new" && next === "(",
          parsePrefix: (parser) => {
            ...
            } else if (newKeyword && value.type === "JsdocTypeFunction" && value.
            arrow) {
              result = value;
              ...
              for (const p of result.parameters) {
                if (p.type === "JsdocTypeKeyValue" && !allowNamedParameters.includes(
                p.key)) {
                  throw new Error(`only allowed named parameters are ${allowNamedParameters.
                  join(", ")} but got ${p.type}`);
                }
              }
            }
          ...
          parsePrefix: (parser) => {
            ...
            try {
              ...
              if (e instanceof NoParsletFoundError) {
                if (brackets) {
                  throw new Error("Empty square brackets for variadic are not al\
lowed.");
                }
                return {
                  type: "JsdocTypeVariadic",
      ...
      const symbolParslet = composeParslet({
        ...
        parseInfix: (parser, left) => {
          if (left.type !== "JsdocTypeName") {
            throw new Error("Symbol expects a name on the left side. (Reacting o\
n '(')");
          }
          parser.consume("(");
          const result = {
          ...
          parsePrefix: (parser) => {
            ...
            if (!parser.consume("}")) {
              ...
              while (true) {
                ...
                if (field.type === "JsdocTypeNumber" || field.type === "JsdocTyp\
eName" || field.type === "JsdocTypeStringValue") {
                  let quote2;
                ...
                } else if (field.type === "JsdocTypeObjectField" || field.type ===
              ...
              result.meta.separator = separator !== null && separator !== void 0 ?
              separator : "comma";
              if (!parser.consume("}")) {
      ...
      function createObjectFieldParslet({ allowSquaredProperties, allowKeyTypes,
      allowReadonly, allowOptional }) {
        return composeParslet({
          ...
          parseInfix: (parser, left) => {
            ...
            const parentParser = (_a = parser.baseParser) !== null && _a !== void 0 ?
            _a : parser;
            parentParser.acceptLexerState(parser);
            if (left.type === "JsdocTypeNumber" || left.type === "JsdocTypeName" ||
            left.type === "JsdocTypeStringValue" || isSquaredProperty(left)) {
      ...
      function createKeyValueParslet({ allowOptional, allowVariadic }) {
        return composeParslet({
          ...
          parseInfix: (parser, left) => {
            ...
            if (allowVariadic && left.type === "JsdocTypeVariadic" && left.element !==
            void 0) {
              variadic = true;
              left = left.element;
            }
      ...
      const assertsParslet = composeParslet({
        ...
        parsePrefix: (parser) => {
          ...
          if (left.type !== "JsdocTypeName") {
            throw new UnexpectedTypeError(left, "A typescript asserts always has\
 to have a name on the left side.");
          }
          parser.consume("is");
          return {
      ...
      function createTupleParslet({ allowQuestionMark }) {
        return composeParslet({
          ...
          parsePrefix: (parser) => {
            ...
            if (!allowQuestionMark && result.elements.some((e) => e.type === "Js\
docTypeUnknown")) {
      ...
      const predicateParslet = composeParslet({
        ...
        parseInfix: (parser, left) => {
          if (left.type !== "JsdocTypeName") {
            throw new UnexpectedTypeError(left, "A typescript predicate always h\
as to have a name on the left side.");
          }
          parser.consume("is");
          return {
      ...
      const objectSquaredPropertyParslet = composeParslet({
        ...
        parsePrefix: (parser) => {
          ...
          if (parser.consume(":")) {
            ...
            throw new Error("Missing ':' or 'in' inside square bracketed propert\
y.");
          }
          if (!parser.consume("]")) {
      ...
      const objectFieldGrammar = [
        ...
        createNameParslet({
          allowedAdditionalTokens: ["module", "event", "keyof", "event", "extern\
al", "in"]
        }),
        nullableParslet,
        optionalParslet,
      ...
      function transform(rules2, parseResult) {
        ...
        if (rule === void 0) {
          throw new Error(`In this set of transform rules exists no rule for typ\
e ${parseResult.type}.`);
        }
        return rule(parseResult, (aParseResult) => transform(rules2, aParseResult));
      }
      function notAvailableTransform(parseResult) {
        throw new Error("This transform is not available. Are you trying the cor\
rect parsing mode?");
      }
      ...
      function stringifyRules() {
        return {
          JsdocTypeParenthesis: (result, transform2) => `(${result.element !== void 0 ?
          ...
          JsdocTypeFunction: (result, transform2) => {
            if (!result.arrow) {
              ...
              let stringified = `(${result.parameters.map(transform2).join(", ")}\
          ...
          JsdocTypeTuple: (result, transform2) => `[${result.elements.map(transform2).
          join(", ")}]`,
          JsdocTypeVariadic: (result, transform2) => result.meta.position === void 0 ?
          "..." : applyPosition(result.meta.position, transform2(result.element),
          "..."),
          ...
          JsdocTypeGeneric: (result, transform2) => {
            if (result.meta.brackets === "square") {
              ...
              if (element.type === "JsdocTypeUnion" || element.type === "JsdocTy\
peIntersection") {
                return `(${transformed})[]`;
              } else {
                return `${transformed}[]`;
              ...
              return `${transform2(result.left)}${result.meta.dot ? "." : ""}<${result.
              elements.map(transform2).join(", ")}>`;
            }
          },
          ...
          JsdocTypeSpecialNamePath: (result) => `${result.specialType}:${quote(result.
          value, result.meta.quote)}`,
          JsdocTypeNotNullable: (result, transform2) => applyPosition(result.meta.
          position, transform2(result.element), "!"),
          JsdocTypeNull: () => "null",
          JsdocTypeNullable: (result, transform2) => applyPosition(result.meta.position,
          transform2(result.element), "?"),
          JsdocTypeNumber: (result) => result.value.toString(),
          JsdocTypeObject: (result, transform2) => `{${result.elements.map(transform2).
          join((result.meta.separator === "comma" ? "," : ";") + " ")}}`,
          JsdocTypeOptional: (result, transform2) => applyPosition(result.meta.position,
          transform2(result.element), "="),
          JsdocTypeSymbol: (result, transform2) => `${result.value}(${result.element !==
          void 0 ? transform2(result.element) : ""})`,
          JsdocTypeTypeof: (result, transform2) => `typeof ${transform2(result.element)}`,
          JsdocTypeUndefined: () => "undefined",
          JsdocTypeUnion: (result, transform2) => result.elements.map(transform2).
          join(" | "),
          JsdocTypeUnknown: () => "?",
          JsdocTypeIntersection: (result, transform2) => result.elements.map(transform2).
          join(" & "),
          JsdocTypeProperty: (result) => quote(result.value, result.meta.quote),
          JsdocTypePredicate: (result, transform2) => `${transform2(result.left)}\
 is ${transform2(result.right)}`,
          JsdocTypeIndexSignature: (result, transform2) => `[${result.key}: ${transform2(
          result.right)}]`,
          JsdocTypeMappedType: (result, transform2) => `[${result.key} in ${transform2(
          result.right)}]`,
          JsdocTypeAsserts: (result, transform2) => `asserts ${transform2(result.
          left)} is ${transform2(result.right)}`
        };
      }
      ...
      const catharsisTransformRules = {
        ...
        JsdocTypeVariadic: (result, transform2) => {
          if (result.element === void 0) {
            throw new Error("dots without value are not allowed in catharsis mod\
e");
          }
          const transformed = transform2(result.element);
        ...
        JsdocTypeStringValue: (result) => makeName(quote(result.value, result.meta.
        quote)),
        JsdocTypeUndefined: () => ({
        ...
        JsdocTypeSpecialNamePath: (result) => makeName(result.specialType + ":" +
        quote(result.value, result.meta.quote)),
        ...
        JsdocTypeObject: (result, transform2) => {
          ...
          for (const field of result.elements) {
            if (field.type !== "JsdocTypeObjectField" && field.type !== "JsdocTy\
peJsdocObjectField") {
        ...
        JsdocTypeNamePath: (result, transform2) => {
          ...
          const joiner = result.pathType === "inner" ? "~" : result.pathType ===
          "instance" ? "#" : ".";
        ...
        JsdocTypeSymbol: (result) => {
          ...
          "JsdocTypeName") {
            value += element.value;
          } else if ((element === null || element === void 0 ? void 0 : element.
          type) === "JsdocTypeNumber") {
        ...
        JsdocTypeParenthesis: (result, transform2) => transform2(assertRootResult(
        result.element)),
      ...
      const jtpRules = {
        JsdocTypeOptional: (result, transform2) => ({
          ...
          meta: {
            syntax: result.meta.position === "prefix" ? "PREFIX_EQUAL_SIGN" : "S\
UFFIX_EQUALS_SIGN"
          }
        }),
        JsdocTypeNullable: (result, transform2) => ({
          ...
          value: transform2(result.element),
          meta: {
            syntax: result.meta.position === "prefix" ? "PREFIX_QUESTION_MARK" :
        ...
        JsdocTypeNotNullable: (result, transform2) => ({
          ...
          value: transform2(result.element),
          meta: {
            syntax: result.meta.position === "prefix" ? "PREFIX_BANG" : "SUFFIX_\
BANG"
          }
        }),
        JsdocTypeVariadic: (result, transform2) => {
          const transformed = {
            type: "VARIADIC",
            meta: {
              syntax: result.meta.position === "prefix" ? "PREFIX_DOTS" : result.
              meta.position === "suffix" ? "SUFFIX_DOTS" : "ONLY_DOTS"
            }
          };
          if (result.element !== void 0) {
            ...
            params: specialParams.params.map((param) => {
              if (param.type === "JsdocTypeKeyValue") {
                if (param.right === void 0) {
                  throw new Error("Function parameter without ':' is not expecte\
d to be 'KEY_VALUE'");
                }
                return {
        ...
        JsdocTypeGeneric: (result, transform2) => {
          const transformed = {
            ...
            meta: {
              syntax: result.meta.brackets === "square" ? "SQUARE_BRACKET" : result.
              meta.dot ? "ANGLE_BRACKET_WITH_DOT" : "ANGLE_BRACKET"
            }
          };
          if (result.meta.brackets === "square" && result.elements[0].type === "\
JsdocTypeFunction" && !result.elements[0].parenthesis) {
...
peJsdocObjectField") {
        ...
        JsdocTypeSpecialNamePath: (result) => {
          if (result.specialType !== "module") {
            throw new Error(`jsdoctypeparser does not support type ${result.specialType}\
 at this point.`);
          }
          return {
            type: "MODULE",
        ...
        JsdocTypeNamePath: (result, transform2) => {
          ...
          let name;
          let quoteStyle;
          if (result.right.type === "JsdocTypeSpecialNamePath" && result.right.specialType ===
          "event") {
            hasEventPrefix = true;
        ...
        JsdocTypeUnion: (result, transform2) => nestResults("UNION", result.elements.
        map(transform2)),
        ...
        JsdocTypeIntersection: (result, transform2) => nestResults("INTERSECTION",
        result.elements.map(transform2)),
      ...
      function _traverse(node, parentNode, property, onEnter, onLeave) {
        onEnter === null || onEnter === void 0 ? void 0 : onEnter(node, parentNode,
        property);
        ...
        onLeave === null || onLeave === void 0 ? void 0 : onLeave(node, parentNode,
        property);
      }
...
o(K, "splitLines");
function X(e = {}) {
  return Object.assign({ tag: "", name: "", type: "", optional: false, description: "",
  problems: [], source: [] }, e);
}
o(X, "seedSpec");
function F(e = {}) {
...
function J({ fence: e = "```" } = {}) {
  ...
  return /* @__PURE__ */ o(function(s) {
    let i = [[]], p = false;
    for (let a of s)
      Fe.test(a.tokens.description) && !p ? i.push([a]) : i[i.length - 1].push(a),
      p = r(a.tokens.description, p);
    return i;
  }, "parseBlock");
}
o(J, "getParser");
...
function N({ startLine: e = 0, markers: t = f } = {}) {
  ...
  return /* @__PURE__ */ o(function(i) {
    let p = i, a = F();
    if ([a.lineEnd, p] = G(p), [a.start, p] = y(p), r === null && p.startsWith(t.
    start) && !p.startsWith(t.nostart) && (r = [], a.delimiter = p.slice(0, t.start.
    length), p = p.slice(t.start.length), [a.postDelimiter, p] = y(p)), r === null)
      return n++, null;
    let c = p.trimRight().endsWith(t.end);
    if (a.delimiter === "" && p.startsWith(t.delim) && !p.startsWith(t.end) && (a.
    delimiter = t.delim, p = p.slice(t.delim.length), [
      a.postDelimiter,
      p
    ] = y(p)), c) {
      let u = p.trimRight();
...
function R({ tokenizers: e }) {
  return /* @__PURE__ */ o(function(r) {
    ...
    let s = X({ source: r });
    for (let i of e)
      if (s = i(s), !((n = s.problems[s.problems.length - 1]) === null || n === void 0) &&
      n.critical)
        break;
    return s;
  }, "parseSpec");
...
function P() {
  return (e) => {
    ...
    return r === null ? (e.problems.push({
      ...
      critical: true
    }), e) : (t.tag = r[1], t.postTag = r[3], t.description = t.description.slice(
    r[0].length), e.tag = r[2], e);
  };
}
o(P, "tagTokenizer");
function S(e = "compact") {
  ...
  return (r) => {
    ...
    for (let [a, [c, u]] of s.entries())
      c.type = u, a > 0 && (c.type = c.postDelimiter.slice(p) + u, c.postDelimiter =
      c.postDelimiter.slice(0, p)), [c.postType, c.description] = y(c.description.
      slice(u.length)), i.push(c.type);
    return i[0] = i[0].slice(1), i[i.length - 1] = i[i.length - 1].slice(0, -1),
    r.type = t(i), r;
  };
}
o(S, "typeTokenizer");
...
function Re(e) {
  return e === "compact" ? (t) => t.map(Ne).join("") : e === "preserve" ? (t) => t.
  join(`
`) : e;
}
o(Re, "getJoiner");
var Ae = /* @__PURE__ */ o((e) => e && e.startsWith('"') && e.endsWith('"'), "is\
Quoted");
function O() {
  let e = /* @__PURE__ */ o((t, { tokens: r }, n) => r.type === "" ? t : n, "typ\
eEnd");
  return (t) => {
    let { tokens: r } = t.source[t.source.reduce(e, 0)], n = r.description.trimLeft(),
    s = n.split('"');
    if (s.length > 1 && s[0] === "" && s.length % 2 === 1)
      return t.name = s[1], r.name = `"${s[1]}"`, [r.postName, r.description] = y(
      n.slice(r.name.length)), t;
    let i = 0, p = "", a = false, c;
    for (let m of n) {
      if (i === 0 && k(m))
        break;
    ...
    if (p[0] === "[" && p[p.length - 1] === "]") {
      ...
      let m = p.split("=");
      if (p = m[0].trim(), m[1] !== void 0 && (c = m.slice(1).join("=").trim()),
      p === "")
        return t.problems.push({
    ...
    return t.optional = a, t.name = p, r.name = u, c !== void 0 && (t.default = c),
    [r.postName, r.description] = y(n.slice(r.name.length)), t;
  };
}
o(O, "nameTokenizer");
...
o(A, "getJoiner");
function Ve(e, t = f) {
  return e.map(({ tokens: { description: r } }) => r.trim()).filter((r) => r !==
  "").join(" ");
}
o(Ve, "compactJoiner");
var Ce = /* @__PURE__ */ o((e, { tokens: t }, r) => t.type === "" ? e : r, "line\
No");
var _e = /* @__PURE__ */ o(({ tokens: e }) => (e.delimiter === "" ? e.start : e.
postDelimiter.slice(1)) + e.description, "getDescription");
function Be(e, t = f) {
  if (e.length === 0)
    return "";
  e[0].tokens.description === "" && e[0].tokens.delimiter === t.start && (e = e.
  slice(1));
  let r = e[e.length - 1];
  return r !== void 0 && r.tokens.description === "" && r.tokens.end.endsWith(t.
  end) && (e = e.slice(0, -1)), e = e.slice(e.reduce(Ce, 0)), e.map(_e).join(`
`);
}
o(Be, "preserveJoiner");
function V({ startLine: e = 0, fence: t = "```", spacing: r = "compact", markers: n = f,
tokenizers: s = [
  P(),
  S(r),
  O(),
  ...
  let i = N({ startLine: e, markers: n }), p = J({ fence: t }), a = R({ tokenizers: s }),
  c = A(r);
  return function(u) {
    let m = [];
    for (let ge of K(u)) {
...
o(V, "getParser");
function Ie(e) {
  return e.start + e.delimiter + e.postDelimiter + e.tag + e.postTag + e.type + e.
  postType + e.name + e.postName + e.description + e.end + e.lineEnd;
}
o(Ie, "join");
function C() {
...
o(He, "extractReturns");
var g = distExports.stringifyRules();
...
function Ze({ name: e, raw: t, elements: r }) {
  return r != null ? l(r.map(se).join(" | ")) : t != null ? l(t.replace(/^\|\s*/,
  "")) : l(e);
}
o(Ze, "generateUnion");
...
function it(e) {
  return e ? e.name === "string" ? true : e.name === "enum" ? Array.isArray(e.value) &&
  e.value.every(
...
var gt = /* @__PURE__ */ ((n) => (n.AUTO = "auto", n.CODE = "code", n.DYNAMIC = "\
dynamic", n))(gt || {});
export {
  cn as c,
  gt as g,
