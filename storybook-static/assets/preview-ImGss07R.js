...
var clearStyle = (input) => {
  let selector = typeof input == "string" ? input : input.join(""), element = global.
...
var addOutlineStyles = (selector, css) => {
  ...
  css);
    ...
    style.setAttribute("id", selector), style.innerHTML = css, global.document.head.
    appendChild(style);
  }
};
...
var withOutline = (StoryFn, context) => {
  let { globals } = context, isActive = [true, "true"].includes(globals[PARAM_KEY]),
  isInDocs = context.viewMode === "docs", outlineStyles = useMemo(() => outlineCSS(
  isInDocs ? '[data-story-block="true"]' : ".sb-show-main"), [context]);
  return useEffect(() => {
    let selectorId = isInDocs ? `addon-outline-docs-${context.id}` : "addon-outl\
ine";
    return isActive ? addOutlineStyles(selectorId, outlineStyles) : clearStyles(
    selectorId), () => {
